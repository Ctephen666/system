...
class AromaManager:
    ...
    async def start(self, requirement: str) -> dict[str, Any]:
        ...
        if not self._config.get_config("AROMA.ENABLED", False):
            return self._error(
                "hardware_disabled",
        ...
        self._task = asyncio.create_task(
            self._run_recipe(recipe, startup, loop), name="aroma:recipe"
        )
        try:
            await asyncio.wait_for(asyncio.shield(startup), timeout=self._startup_timeout())
        except Exception as error:
            ...
            if self._task:
                await self._task
            return self._error("serial_error", f"香薰硬件未能启动: {error}")
    ...
    async def _run_recipe(self, recipe: AromaRecipe, startup, loop) -> None:
        try:
    ...
    def _driver(self) -> Dam1600CRelayDriver:
        return Dam1600CRelayDriver(
            RelaySettings(
                ...
                active_high=bool(self._config.get_config("AROMA.ACTIVE_HIGH", True)),
            )
        )
